...
Usage:
    python support/detect.py [image_path...]

Examples:
    python support/detect.py test1.jpg
    python support/detect.py img1.jpg img2.jpg test_folder/
    python support/detect.py                          # runs on dataset/val/images
"""
...
def collect_targets(args):
    if not args:
        return sorted((BASE / "data" / "val" / "images").iterdir())
    targets = []
    for arg in args:
        p = Path(arg)
        if p.is_dir():
...
def main():
    if not MODEL_PATH.exists():
        print("Model not found. Train first:  python support/train.py")
